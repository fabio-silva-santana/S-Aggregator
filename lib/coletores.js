...
//
// RJ (Firjan) usa Paradigma WBC (portaldecompras.firjan.com.br) — coletado
// via FONTES_PARADIGMA; SENAC/SEBRAE-RJ chegam via PNCP.
// ============================================================
...
export const ENTIDADES = ["SESI", "SENAI", "SESC", "SENAC", "SENAR", "SEBRAE", "SEST/SENAT", "SESCOOP", "Correios", "Sistema Indústria", "Sistema Comércio"];
...
// Siglas das federações estaduais da indústria (compram para SESI/SENAI/IEL)
const SIGLAS_FIE = new Set([
  "FIEG", "FIEPE", "FIESP", "FIRJAN", "FIEB", "FIEC", "FIEMG", "FIESC", "FIERGS",
  "FINDES", "FIEA", "FIERO", "FIEAC", "FIEAM", "FIEAP", "FIEMA", "FIEMT", "FIEMS",
  "FIEPA", "FIEPB", "FIEPI", "FIERN", "FIETO", "FIER", "FIEP",
]);
...
export function detectarEntidade(nomes) {
  ...
  // SESCOOP contém "SESC" — testar antes; OCB administra o SESCOOP
  if (t.includes("SESCOOP") || t.includes("COOPERATIVISMO") || /\bOCB\b/.test(t) || t.includes("ORGANIZACAO DAS COOPERATIVAS") || t.includes("ORGANIZAÇÃO DAS COOPERATIVAS")) return "SESCOOP";
  if (/\bSEST\b/.test(t) || t.includes("SENAT") || t.includes("SOCIAL DO TRANSPORTE") || t.includes("APRENDIZAGEM DO TRANSPORTE") || t.includes("CONFEDERACAO NACIONAL DO TRANSPORTE") || t.includes("CONFEDERAÇÃO NACIONAL DO TRANSPORTE") || /\bCNT\b/.test(t)) return "SEST/SENAT";
  ...
  // Fecomércio/CNC administram SESC e SENAC — sistema do comércio
  if (t.includes("FECOMERCIO") || t.includes("FECOMÉRCIO") || t.includes("FEDERACAO DO COMERCIO") || t.includes("FEDERAÇÃO DO COMÉRCIO") || t.includes("CONFEDERACAO NACIONAL DO COMERCIO") || t.includes("CONFEDERAÇÃO NACIONAL DO COMÉRCIO")) return "Sistema Comércio";
  if (t.includes("SESC") || t.includes("SOCIAL DO COMERCIO") || t.includes("SOCIAL DO COMÉRCIO")) return "SESC";
  // CNA e FAEs (Federações da Agricultura) administram o SENAR
  if (/\bSENAR\b/.test(t) || t.includes("APRENDIZAGEM RURAL") || t.includes("FEDERACAO DA AGRICULTURA") || t.includes("FEDERAÇÃO DA AGRICULTURA") || t.includes("CONFEDERACAO DA AGRICULTURA") || t.includes("CONFEDERAÇÃO DA AGRICULTURA")) return "SENAR";
  const sesi = /\bSESI\b/.test(t) || t.includes("SOCIAL DA INDUSTRIA") || t.includes("SOCIAL DA INDÚSTRIA");
  const senai = t.includes("SENAI") || t.includes("APRENDIZAGEM INDUSTRIAL");
  if (sesi && senai) return "SESI e SENAI";
  if (sesi) return "SESI";
  if (senai) return "SENAI";
  // Federações da indústria e CNI compram para o sistema (SESI/SENAI/IEL)
  if (t.includes("SISTEMA IND") || t.includes("FEDERACAO DAS INDUSTRIAS") || t.includes("FEDERAÇÃO DAS INDÚSTRIAS") || t.includes("CONFEDERACAO NACIONAL DA INDUSTRIA") || t.includes("CONFEDERAÇÃO NACIONAL DA INDÚSTRIA") || /\bIEL\b/.test(t)) return "Sistema Indústria";
  for (const sigla of SIGLAS_FIE) {
    if (new RegExp(`\\b${sigla}\\b`).test(t)) return "Sistema Indústria";
  }
  return null;
}
...
async function coletarPNCP(termo) {
  ...
  for (let pagina = 1; pagina <= 2; pagina++) {
    ...
    editais.push(...mapearPNCP(data.items || []));
    if ((data.items || []).length < 100) break;
  }
  return editais;
}

// Normaliza um item do buscador do PNCP para o formato de edital do app.
// Devolve null quando o órgão não é do Sistema S/Correios ou a UF é inválida.
// Exportada: usada tanto pela coleta ao vivo quanto pela colheita por CNPJ
// (lib/harvestPNCP.js).
export function normalizarItemPNCP(item) {
  const entidade = detectarEntidade(item.orgao_nome);
  if (!entidade) return null;
  if (!UFS.includes(item.uf)) return null;
...
// SESI/SENAI já têm coletor dedicado (mais rico) nessas UFs — o merge
// descarta a versão PNCP para evitar duplicidade.
export function ehIndustriaComColetorDedicado(e) {
  return ENTIDADES_INDUSTRIA.has(e.entidade) && UFS_COM_COLETOR_DEDICADO_INDUSTRIA.has(e.uf);
}

function mapearPNCP(items) {
  return items.map((i) => normalizarItemPNCP(i)).filter((e) => e && !ehIndustriaComColetorDedicado(e));
}

// ---------- Fonte 3b: Paradigma WBC (compras.sestsenat.org.br) ----------
//
// Plataforma de e-procurement usada pelo SEST/SENAT (portal nacional único —
// as unidades operacionais de todas as UFs publicam nele) e também pela
// Firjan-RJ. O mural de editais expõe um webservice ASMX que responde JSON
// sem exigir sessão: POST /portal/WebService/Servicos.asmx/PesquisarProcessos
// com o dto abaixo (tmpTipoMuralProcesso=2 = mural de editais).

const FONTES_PARADIGMA = [
  {
    id: "sest-senat",
    host: "https://compras.sestsenat.org.br",
    entidadePadrao: "SEST/SENAT",
    regional: "SEST/SENAT (portal nacional)",
    portal: "https://compras.sestsenat.org.br/portal/Mural.aspx?nNmTela=E",
    paginas: 3, // 3 × 50 editais mais recentes
    ufFixa: null, // extrai a UF do sufixo "/UF" da unidade
  },
  {
    id: "firjan",
    host: "https://portaldecompras.firjan.com.br",
    entidadePadrao: "SESI e SENAI",
    regional: "SESI/SENAI-RJ (Firjan)",
    portal: "https://portaldecompras.firjan.com.br/portal/Mural.aspx?nNmTela=E",
    paginas: 3,
    ufFixa: "RJ", // Firjan é a federação das indústrias do RJ — tudo é RJ
  },
  // Federações estaduais da indústria (SESI/SENAI) em Paradigma WBC
  { id: "fiemg", host: "https://compras.fiemg.com.br", entidadePadrao: "SESI e SENAI", regional: "SESI/SENAI-MG (FIEMG)", portal: "https://compras.fiemg.com.br/portal/Mural.aspx?nNmTela=E", paginas: 3, ufFixa: "MG" },
  { id: "fiesc", host: "https://portaldecompras.fiesc.com.br", entidadePadrao: "SESI e SENAI", regional: "SESI/SENAI-SC (FIESC)", portal: "https://portaldecompras.fiesc.com.br/portal/Mural.aspx?nNmTela=E", paginas: 3, ufFixa: "SC" },
  { id: "fiec", host: "https://portaldecompras.sfiec.org.br", entidadePadrao: "SESI e SENAI", regional: "SESI/SENAI-CE (FIEC)", portal: "https://portaldecompras.sfiec.org.br/portal/Mural.aspx?nNmTela=E", paginas: 3, ufFixa: "CE" },
  { id: "findes", host: "https://portaldecompras.findes.org.br", entidadePadrao: "SESI e SENAI", regional: "SESI/SENAI-ES (FINDES)", portal: "https://portaldecompras.findes.org.br/portal/Mural.aspx?nNmTela=E", paginas: 3, ufFixa: "ES" },
  { id: "fiems", host: "https://compras.fiems.com.br", entidadePadrao: "SESI e SENAI", regional: "SESI/SENAI-MS (FIEMS)", portal: "https://compras.fiems.com.br/portal/Mural.aspx?nNmTela=E", paginas: 3, ufFixa: "MS" },
  { id: "fieb", host: "https://compras.fieb.org.br", entidadePadrao: "SESI e SENAI", regional: "SESI/SENAI-BA (FIEB)", portal: "https://compras.fieb.org.br/portal/Mural.aspx?nNmTela=E", paginas: 3, ufFixa: "BA" },
  // SESC regionais em Paradigma SCR360 (host multi-tenant scr360.paradigmabs.com.br/<slug>)
  { id: "sesc-sp", host: "https://scr360.paradigmabs.com.br/sescsp", entidadePadrao: "SESC", regional: "SESC-SP", portal: "https://scr360.paradigmabs.com.br/sescsp/portal/Mural.aspx", paginas: 3, ufFixa: "SP" },
];

function ufDaUnidadeParadigma(nomeEmpresa, ufFixa) {
  if (ufFixa) return ufFixa;
  // "B 077 - SEST - MARABA/PA" → PA; sede nacional → DF
  const m = /\/([A-Z]{2})\s*$/.exec((nomeEmpresa || "").trim());
  if (m && UFS.includes(m[1])) return m[1];
  return "DF";
}

function tsParadigma(v) {
  // "/Date(1783943849920)/" → ISO (datas negativas = campo vazio)
  const m = /-?\d+/.exec(v || "");
  if (!m) return null;
...
// Uma página do mural Paradigma, com 3 tentativas e backoff progressivo.
// Esses portais falham de forma transitória sob carga ("fetch failed"), então
// a re-tentativa evita perder a fonte inteira por um soluço de rede.
async function paginaParadigma(fonte, p) {
  const dto = {
    ...
    nCdTipoProcesso: 0, nCdEmpresa: 0, sNrProcesso: "", nCdProcesso: 0, sDsObjeto: "",
    sDtPeriodoDe: "", sDtPeriodoAte: "", sOrdenarPor: "NCDPROCESSO", sOrdenarPorDirecao: "DESC",
    dtoPaginacao: { nPaginaDe: p * 50 + 1, nPaginaAte: (p + 1) * 50 },
    dtoIdioma: { nCdIdioma: 1 },
  };
  let ultimoErro;
  // 2 tentativas com timeout curto: quando o portal responde, leva <1s; quando
  // barra o IP do servidor, não adianta insistir — não pagamos o preço da espera.
  for (let t = 0; t < 2; t++) {
    if (t) await new Promise((res) => setTimeout(res, 800));
    try {
      const r = await fetchComTimeout(`${fonte.host}/portal/WebService/Servicos.asmx/PesquisarProcessos`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json; charset=utf-8",
          "User-Agent": "Mozilla/5.0 (S-Aggregator; radar Sistema S)",
          Accept: "application/json",
        },
        body: JSON.stringify({ dtoProcesso: dto }),
        next: { revalidate: REVALIDATE },
      }, 9000);
      if (!r.ok) throw new Error(`HTTP ${r.status} no mural Paradigma (${fonte.id})`);
...
async function coletarParadigma(fonte) {
  const editais = [];
  for (let p = 0; p < fonte.paginas; p++) {
    let itens;
    try {
      itens = await paginaParadigma(fonte, p);
    } catch (e) {
      if (p === 0) throw e; // 1ª página é obrigatória; as demais são bônus
      break;
    }
    if (!itens.length) break;
    for (const item of itens) {
      const objeto = (item.sDsObjeto || "").trim();
      if (!objeto || !item.nCdProcesso) continue;
      const dataAbertura = tsParadigma(item.tDtFinal) || tsParadigma(item.tDtInicial);
      const encerrado = Boolean(tsParadigma(item.tDtEncerrado));
      // o mural de editais lista processos em divulgação: sem data de encerramento,
      // publicação recente (≤45d) = recebendo propostas
      const limiteAberto = new Date(Date.now() - 45 * 86400000).toISOString().slice(0, 10);
      const status = encerrado
        ? "Encerrado"
        : (dataAbertura && dataAbertura >= hojeISO()) || (tsParadigma(item.tDtInicial) || "") >= limiteAberto
          ? "Aberto"
          : "Em andamento";
      const entidade = detectarEntidade(item.sNmEmpresa) || fonte.entidadePadrao;
      editais.push({
        id: `pd-${fonte.id}-${item.nCdProcesso}`,
        numero: item.sNrEdital || item.sNrProcessoDisplay || String(item.nCdProcesso),
        entidade,
        uf: ufDaUnidadeParadigma(item.sNmEmpresa, fonte.ufFixa),
        regional: (item.sNmEmpresa || fonte.regional).slice(0, 80),
        cidade: null,
        modalidade: normalizarModalidade(item.sNmModalidade),
        modalidadeOriginal: item.sNmModalidade || null,
        segmento: classificarSegmento(objeto),
        objeto: objeto.slice(0, 1200),
        titulo: item.sNrEdital || null,
        valorEstimado: item.dVlEstimado > 0 ? String(item.dVlEstimado) : null,
        valorContratado: item.dVlNegociado > 0 ? String(item.dVlNegociado) : null,
        dataPublicacao: tsParadigma(item.tDtInicial),
        dataAbertura,
        dataHomologacao: tsParadigma(item.tDtEncerrado),
        status,
        statusOriginal: item.sDsImagem || null,
        fase: inferirFase(item.sDsImagem, status),
        criterioJulgamento: null,
        participantes: [],
        vencedores: [],
        lotes: [],
        documentos: [],
        portal: fonte.portal,
        fonte: `Portal de compras — ${fonte.regional}`,
      });
    }
...
export async function coletarEditais() {
  // PNCP: preferimos a colheita profunda noturna (por CNPJ de cada órgão do
  // Sistema S, gravada no banco pelo cron /api/cron/coleta) — cobre as 27 UFs
  // sem o viés de recência da busca textual. Sem banco/colheita, cai na
  // busca textual ao vivo (cobertura parcial, comportamento antigo).
  const { lerEditaisPNCPBanco } = await import("@/lib/harvestPNCP");
  const pncpBanco = await lerEditaisPNCPBanco();

  const tarefas = [
    ...FONTES_DJANGO.map((f) => ({ nome: f.regional, promessa: coletarDjango(f) })),
    ...FONTES_STW.map((f) => ({ nome: f.dep, promessa: coletarSTW(f) })),
    ...FONTES_PARADIGMA.map((f) => ({ nome: f.regional, promessa: coletarParadigma(f), link: f.portal })),
    ...(pncpBanco
      ? [{
          nome: `PNCP — colheita por órgão (${new Date(pncpBanco.coletadoEm).toLocaleDateString("pt-BR")})`,
          promessa: Promise.resolve(pncpBanco.editais.filter((e) => !ehIndustriaComColetorDedicado(e))),
        }]
  ...
  resultados.forEach((r, i) => {
    if (r.status === "fulfilled") {
      ...
      // Alguns portais (ex.: FIEMS) respondem de IP residencial mas barram IPs de
      // datacenter — do servidor isso chega como "fetch failed". Quando a fonte
      // tem portal público, mostramos status transparente + link em vez do erro cru.
      const msg = String(r.reason?.message || r.reason);
      const pareceBloqueio = /fetch failed|ECONNRESET|ETIMEDOUT|aborted|certificate/i.test(msg);
      fontes.push(
        tarefas[i].link && pareceBloqueio
          ? {
              nome: tarefas[i].nome,
              ok: false,
              erro: "Portal indisponível para acesso automatizado a partir do servidor — consulte os editais direto no site oficial.",
              link: tarefas[i].link,
            }
          : { nome: tarefas[i].nome, ok: false, erro: msg.slice(0, 120), link: tarefas[i].link }
      );
    }
  });
