...
const PAUSA_LOTE_MS = 350;
const PAGINAS_MAX = 2; // 2 × 100 por órgão
...
async function carregarRegistro(sb) {
  const mapa = new Map(ORGAOS_PNCP_SEED.map((o) => [o.cnpj, { ...o }]));
  const { data } = await sb.from("orgaos_pncp").select("cnpj, nome, entidade, uf");
  for (const o of data || []) {
    const atual = mapa.get(o.cnpj);
    if (atual) { if (!atual.uf && o.uf) atual.uf = o.uf; }
...
export async function colherPNCP() {
  ...
  // atualiza o registro de órgãos e remove editais que sumiram desta colheita
  if (atualizacoesOrgao.length) await sb.from("orgaos_pncp").upsert(atualizacoesOrgao, { onConflict: "cnpj" });
  // só limpa os antigos se a colheita foi majoritariamente bem-sucedida (evita esvaziar o banco numa noite ruim)
  if (orgaosOk >= registro.length * 0.7) {
    await sb.from("editais_pncp").delete().lt("updated_at", inicio);
  }

  return { ok: falhas.length === 0, orgaos: registro.length, orgaosOk, editais: totalEditais, falhas: falhas.slice(0, 12), coletadoEm: inicio };
}
